...
# Extracted data for the 15 valid matches
E_TeV  = [79.76, 78.45, 64.05, 168.32, 70.79, 136.87, 62.98, 83.37, 261.23, 138.47, 191.39, 191.39, 191.39, 133.85, 95.12]
dt_days = [2.14, 1.86, 0.85, 2.65, 1.25, 1.57, 0.27, 2.50, 1.35, 1.00, 0.72, 1.01, 1.71, 2.88, 1.40]
dt_sec = dt_days .* 86400.0 # Convert days to seconds

# 2 measured redshifts, 13 missing (-999.0)
measured_z_array = [-999.0, -999.0, 1.38, -999.0, -999.0, -999.0, -999.0, -999.0, 0.645, -999.0, -999.0, -999.0, -999.0, -999.0, -999.0]

# T90 parameters (2 short GRBs <= 2.0s, 13 long GRBs > 2.0s or missing)
t90_array = [38.66, 33.28, 25.40, 90.50, 4.61, 0.13, 2.43, 0.10, 36.10, 127.75, 25.86, 999.0, 26.37, 70.40, 24.58]
...
for i in 1:N_sims
    ...
    sim_K = [E_TeV[j] * (D_z(sim_z[j]) / D1) for j in 1:15]
...
K_phys_plot = [E_TeV[i] * (D_z(phys_z_plot[i]) / D1) for i in 1:15]
...
p1 = scatter(K_phys_plot, dt_sec, xerror=K_phys_err_plot, color=:darkgreen, markersize=6, 
             label="15-Event MC", legend=:bottomright)
...
plot!(p1; plot_opts...)
savefig(p1, "4. Extended_15_Events.png")

println("\nSuccessfully rendered! Saved Extended_15_Events.png")